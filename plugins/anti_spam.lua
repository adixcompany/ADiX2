  ...
  if msg.from.type == 'user' then
    ...
    if msgs > max_msg then
	  ...
	  if msg.to.type == 'user' then
		...
		if msgs >= max_msg then
			print("Pass2")
			send_large_msg("user#id"..msg.from.id, "کاربر ["..msg.from.id.."] مسدود شد.")
	  ...
	  if msg.to.type == 'chat' or msg.to.type == 'channel' then
		if username then
			...
			send_large_msg(receiver , "در گروه پیام هرز نفرستید.زیرا باعث حذف شما از گروه می شود!\n@"..username.."["..msg.from.id.."] حذف شد\nوضعیت: حذف شده")
		else
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] kicked for #spam")
			send_large_msg(receiver , "در گروه پیام هرز نفرستید.زیرا باعث حذف شما از گروه می شود!\nنام کاربر حذف شده:"..name_log.."["..msg.from.id.."]\nوضعیت: حذف شده")
		end
	  end
      -- incr it on redis
      ...
      if gbanspamonredis then
        if tonumber(gbanspamonredis) ==  4 and not is_owner(msg) then
          ...
          send_large_msg("chat#id"..msg.to.id, "کاربر [ "..name.." ]"..msg.from.id.." از گروه حذف شد به دلیل اسپم دادن!")
		  send_large_msg("channel#id"..msg.to.id, "کاربر [ "..name.." ]"..msg.from.id.." از گروه حذف شد به دلیل اسپم دادن")
          local GBan_log = 'GBan_log'
		  local GBan_log =  data[tostring(GBan_log)]
		  for k,v in pairs(GBan_log) do
			log_SuperGroup = v
			gban_text = "کاربر [ "..name.." ] ( @"..username.." )"..msg.from.id.." حذف شد از طرف ( "..msg.to.print_name.." ) [ "..msg.to.id.." ] به دلیل اسپم دادن!"
