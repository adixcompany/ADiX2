...
local function kick_ban_res(extra, success, result)
	  ...
	  if success == 0 then
		return send_large_msg(receiver, "Error!User not found.Please read catalog in @ad2bc .Error code: XUNF1")
	  end
      ...
      local get_cmd = extra.get_cmd
       if get_cmd == "kick" then
         ...
         if is_momod2(member_id, chat_id) and not is_admin2(sender) then
            send_large_msg(receiver, "Error!You are not a allowed user.Please see bot catalog with /help !Error code: XM1")
			return
         end
      ...
      elseif get_cmd == 'ban' then
        ...
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] بن شد.برای خارج کردن از بن کاتالوگ ربات را با دستور /help بخوانید!')
		ban_user(member_id, chat_id)
      elseif get_cmd == 'unban' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از حالت بن خارج شد')
        local hash =  'banned:'..chat_id
        redis:srem(hash, member_id)
        return 'User '..user_id..' unbanned'
      elseif get_cmd == 'banall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] بصورت سراسری بن شد!در صورتی که مدیری معمولی در ربات هستید حتما با مدیریت اصلی در میان بگذارید.برای خارج کردن از بن کاتالوگ ربات را با دستور /help بخوانید!')
		banall_user(member_id)
      elseif get_cmd == 'unbanall' then
        send_large_msg(receiver, 'کاربر @'..member..' ['..member_id..'] از حالت بن سراسری خارج شد')
 ...
 if matches[1]:lower() == 'id' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == 'ban' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
		...
		send_large_msg(receiver, 'کاربر ['..matches[2]..'] بن شد.برای خارج کردن کاتالوگ ربات را با دستور /help بخوانید')
      else
		local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'ban',
  ...
  if matches[1]:lower() == 'unban' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return 'کاربر '..user_id..' از حالت بن خارج شد'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unban',
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'کاربر ['..user_id..' ] بصورت سراسری بن شد.اگر مدیر اصلی ربات نیستید حتما با مدیریت در میان بگذارید.برای خارج کردن از حالت بن کاتالوگ ربات را با دستور /help بخوانید.'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
    ...
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'کاربر ['..user_id..' ] از حالت بن سراسری خارج شد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
