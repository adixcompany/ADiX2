...
function send_photo_from_url(receiver, url, cb_function, cb_extra)
  ...
  if not file_path then -- Error
    local text = 'Error!System cant download file.Please see bot catalog with /help !Error code: X1'
...
function warns_user_not_allowed(plugin, msg)
  if not user_allowed(plugin, msg) then
    local text = 'Error!You are not a allowed user.Please see bot catalog with /help !Error code: XM1'
...
function ban_list(chat_id)
	...
	local text = "لیست کاربران بن شده: [ID: "..chat_id.." ]:\n\n"
...
function mutes_list(chat_id)
	local hash =  'mute:'..chat_id
	local list = redis:smembers(hash)
	local text = "لیست بی صداها: [ID: "..chat_id.." ]:\n\n"
...
function muted_user_list(chat_id)
	local hash =  'mute_user:'..chat_id
	local list = redis:smembers(hash)
	local text = "کاربران بی صدا شده برای: [ID: "..chat_id.." ]:\n\n"
	for k,v in pairsByKeys(list) do
  		local user_info = redis:hgetall('user:'..v)
		if user_info and user_info.print_name then
			local print_name = string.gsub(user_info.print_name, "_", " ")
...
function Kick_by_reply(extra, success, result)
	...
	if is_momod2(result.from.peer_id, result.to.peer_id) then -- Ignore mods,owner,admin
		return "Error!You cant remove admins.Error code: XM1"
	end
...
function ban_by_reply(extra, success, result)
	...
	if is_momod2(result.from.peer_id, result.to.peer_id) then -- Ignore mods,owner,admin
		...
		send_large_msg(chat, "کاربر "..result.from.peer_id.." بن شد.برای خارج کردن کاربر کاتالوگ ربات را در @ad2bc بخوانید!")
	else
		return
	end
end
...
function ban_by_reply_admins(extra, success, result)
	...
	if is_admin2(result.from.peer_id) then -- Ignore admins
		...
		ban_user(result.from.peer_id, result.to.peer_id)
		send_large_msg(chat, "کاربر "..result.from.peer_id.." بن شد.برای خارج کردن کاربر کاتالوگ ربات را در @ad2bc بخوانید!")
		send_large_msg(channel, "کاربر "..result.from.peer_id.." بن شد.برای خارج کردن کاربر کاتالوگ ربات را در @ad2bc بخوانید!")
	else
		return
	end
end
...
function unban_by_reply(extra, success, result)
	...
	if tonumber(result.from.peer_id) == tonumber(our_id) then -- Ignore bot
		...
		send_large_msg(chat, "کاربر "..result.from.peer_id.." از بن خارج شد")
		-- Save on redis
...
function banall_by_reply(extra, success, result)
	...
	if is_admin2(result.from.peer_id) then -- Ignore admins
		...
		chat_del_user(chat, 'user#id'..result.from.peer_id, ok_cb, false)
		send_large_msg(chat, "کاربر "..name.."["..result.from.peer_id.."] بصورت سراسری بن شد.برای خارج کردن کاربر از بن کاتالوگ ربات را در @ad2bc بخوانید!")
	else
		return
  end
end
